...
* `review 1 \d Nice Condensed Milk Pork Ribs \p 15 \r 4 \w 23/10/2019` : Adds a review to Two Chefs Eating Place with the specified description, price, rating and date.
* `exit` : Exits the app.
...
* For parameters requiring `index`, `index` refers to the index number shown in the displayed main or todo eatery list, or review list. The index *must be a positive integer* 1, 2, 3, ... .
====
...
// tag::load[]
==== Load eateries from other file: `load`

Loads eateries from a json file. +
Format: `load \u [filename]`

****
* Loads eatery data from the specified json file.
